...
File names picked in PPT/Pitch Deck, Image Prompts, and Video Prompts are sanitized before use: control characters, bidi override characters (which can visually disguise one extension as another), and zero-width characters are stripped; `/` and `\` are replaced with `_`, since the name is never used as a path (`..` is otherwise left alone — it is a legitimate character sequence in a file name, e.g. `Q3..final.pdf`); whitespace is collapsed; and the extension is taken from the last segment of the full sanitized name and checked case-insensitively against an allow list in JavaScript, not just the input's `accept` attribute hint. Only after a valid extension is confirmed is the name truncated to 200 characters, trimming the base name so the extension is always preserved.
